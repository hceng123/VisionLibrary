...
public:
    ...
    static float intervalAverage(const cv::cuda::GpuMat& matInput, int interval, cv::cuda::Stream& stream = cv::cuda::Stream::Null());
    static float intervalRangeAverage(const cv::cuda::GpuMat& matInput, int interval, float rangeStart, float rangeEnd,
        cv::cuda::Stream& stream = cv::cuda::Stream::Null());
    static void phaseWrapBuffer(cv::cuda::GpuMat& matPhase, cv::cuda::GpuMat& matBuffer, float fShift = 0.f, cv::cuda::Stream& stream = cv::cuda::Stream::Null());
    ...
    static void phaseCorrectionCmp(cv::cuda::GpuMat& matPhase, const cv::cuda::GpuMat& matPhase1, cv::cuda::GpuMat& matPhaseT, int span, cv::cuda::Stream& stream = cv::cuda::Stream::Null());
