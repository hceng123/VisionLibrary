...
/*static*/ const float Unwrap::ONE_HALF_CYLE = ToFloat ( CV_PI );
...
/*static*/ void Unwrap::calib3DBase(const std::vector<cv::Mat> &vecInputImgs, bool bGuassianFilter, bool bReverseSeq, cv::Mat &matK, cv::Mat &matPPz ) {
    ...
    matPPz = calculatePPz ( matX, matY, matBeta );
...
/*static*/ cv::Mat Unwrap::phaseUnwrapSurface(const cv::Mat &matPhase) {    
    ...
    auto vecPhaseResult = CalcUtils::matToVector<DATA_TYPE>( matPhaseResult );
     ...
     cv::compare ( cv::abs ( matDp ), cv::Scalar::all ( ONE_HALF_CYLE ), matAbsUnderPI, cv::CmpTypes::CMP_LT );
     matDp.setTo ( 0, matAbsUnderPI );
     matDp = setBySign ( matDp, OneCycle);
...
/*static*/ void Unwrap::calc3DHeight(const std::vector<cv::Mat> &vecInputImgs, bool bGuassianFilter, bool bReverseSeq, const cv::Mat &matK, const cv::Mat &matPPz, cv::Mat &matHeight) {
    std::vector<cv::Mat> vecConvertedImgs;
    for ( auto &mat : vecInputImgs ) {
        cv::Mat matConvert;
        mat.convertTo ( matConvert, CV_32FC1 );
    ...
    selectPolePair ( vecPosPole, vecNegPole, matResidue.rows, matResidue.cols, vecPoint1, vecPoint2, vecPoint3 );

    VectorOfPoint vecTree = getIntegralTree ( vecPoint1, vecPoint2, vecPoint3 );
...
/*static*/ cv::Mat Unwrap::phaseUnwrapSurfaceTrk ( const cv::Mat &matPhase, const cv::Mat &matBranchCut) {
    ...
    for ( const auto &point : vecRowsNeedToUnwrap )    {
        ...
        matDp = matPhaseDiff;
        cv::compare ( cv::abs ( matDp ), cv::Scalar ( ONE_HALF_CYLE ), matAbsUnderPI, cv::CmpTypes::CMP_LT );
        matDp.setTo ( 0, matAbsUnderPI );
        matDp = setBySign ( matDp, OneCycle );
...
/*static*/ cv::Mat Unwrap::phaseUnwrapSurfaceByRefer(const cv::Mat &matPhase, float Thresh, const cv::Mat &matRef ) {
    float Th = Thresh;
...
//For shadow area
/*static*/ cv::Mat Unwrap::_findUnstablePoint(const std::vector<cv::Mat> &vecInputImgs) {
    int ROWS = vecInputImgs[0].rows;
    int COLS = vecInputImgs[0].cols;
    cv::Mat matCombine;
    ...
    cv::Mat matResult;
    cv::compare ( matDiff, UNSTABLE_DIFF, matResult, cv::CmpTypes::CMP_LT);
    return matResult.reshape ( 1, ROWS );
}
